...
Andrej Karpathy の [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) を AWS 上で実装したデモ。**Fargate + Bun + S3 Files + SQS + ALB** 構成。

clone → `cdk deploy` → S3 にサンプルを投入 → `curl /query` で動作確認できます。

> ⚠️ **検証用デモです。** ALB は HTTP + Bearer token 平文のため、検証以外の用途には使わないでください。

## 構成

- **LLM**: Bedrock (`global.anthropic.claude-sonnet-4-6`)
- **Query** は Bedrock Runtime 直叩き (1-shot + citation 強制)
...
- **IaC**: CDK v2 (VPC + SQS + ALB + Bearer token)
...
**前提**: Node 22+ / Bun 1.3.12+ / Docker / region `ap-northeast-1` / Bedrock で Sonnet 4.6 が有効。
...
# 14 docs の ingest に 20〜25 分 (進捗は CloudWatch Logs)
